...
| `restoreHostControlAction`              | Read-only or sensitive disclosure  |          No |                              No |   Yes |
| `refreshHostLockAction`                 | Read-only or sensitive disclosure  |         Yes |                              No |    No |
| `releaseHostControlAction`              | Read-only or sensitive disclosure  |         Yes |                              No |   Yes |
...
`restoreHostControlAction` is available only when the current verified session retains the original
owner session or presents the signed HttpOnly recovery credential bound to that owner. It rotates
the primary host credential and is not a replacement for forced takeover.
