...
const REHEARSAL_ELIGIBLE_PLAYERS = 12;
const REHEARSAL_SUBMITTED_BALLOTS = 2;
const REHEARSAL_BAN_SELECTIONS = 8;
...
async function expectRehearsalAggregateTotals(page: Page) {
  await expect(
    page.getByText(`Ballots submitted: ${REHEARSAL_SUBMITTED_BALLOTS} / ${REHEARSAL_ELIGIBLE_PLAYERS}`),
  ).toBeVisible();
  await expect(page.getByText(`Ban selections cast: ${REHEARSAL_BAN_SELECTIONS}`)).toBeVisible();
}
...
export async function expectPublicVotingState(
  ...
  await expectRehearsalAggregateTotals(publicPages.stage.page);
  ...
  await expect(
    publicPages.vote.page.getByText(
      `Ballots submitted: ${REHEARSAL_SUBMITTED_BALLOTS} / ${REHEARSAL_ELIGIBLE_PLAYERS}`,
    ),
  ).toBeVisible();
