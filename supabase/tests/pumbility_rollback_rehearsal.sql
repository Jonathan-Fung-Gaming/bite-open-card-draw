...
begin
  ...
  if (select count(*) from pumbility.data_sources) <> 1
     ...
     or exists (select 1 from pumbility.data_sources where source_key <> 'piu-scores')
     or exists (
...
begin
  ...
  if not exists (select 1 from pg_catalog.pg_class where oid = 'public.rounds'::regclass)
     or not exists (
       select 1 from pg_catalog.pg_class where oid = 'public.protein_profiles'::regclass
     )
     or not exists (
       select 1 from pg_catalog.pg_class where oid = 'public.karaoke_rooms'::regclass
     ) then
...
do $$
begin
  if not exists (select 1 from pg_catalog.pg_namespace where nspname = 'pumbility')
     or not exists (select 1 from storage.buckets where id = 'pumbility-artifacts')
     or not exists (
       select 1 from pg_catalog.pg_roles
       where rolname in ('pumbility_reader', 'pumbility_worker')
     ) then
    raise exception 'Pumbility rollback rehearsal did not restore its transaction';
  end if;
end;
$$;
